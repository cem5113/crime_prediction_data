...
try:
    ...
    print(f"❌ Tren verisi indirilemedi: {e}")
    exit()
...
# === 6. En yakın tren durağına mesafe (metre) ===
...
gdf_crime["distance_to_train_range"] = pd.cut(
    gdf_crime["distance_to_train"], bins=dist_bins, labels=dist_labels, include_lowest=True
)

# === 9. Yakın tren durağı sayısı (dinamik yarıçap) ===
...
gdf_crime["train_stop_count_range"] = pd.cut(
    gdf_crime["train_stop_count"], bins=count_bins, labels=count_labels, include_lowest=True
)
