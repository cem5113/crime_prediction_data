...
geo["distance_to_police_range"] = make_quantile_ranges(
    geo["distance_to_police"], max_bins=5, fallback_label="Unknown"
)
...
try:
    ...
    print(f"⚠️ Parquet önizleme okunamadı: {e}")        os.replace(tmp, path)
        print(f"💾 Kaydedildi: {path}")
    except Exception as e:
        print(f"❌ Kaydetme hatası: {path}\n{e}")
        try:
...
# =============================================================================
# centroid düzeltmesi
# sf_crime_06 panelinde lat/lon yoksa tract_centroids_sf.csv ile tamamla
# =============================================================================
CENTROID_CANDIDATES = [
    str(in_dir / "tract_centroids_sf.csv"),
    str(Path(CRIME_DATA_DIR) / "tract_centroids_sf.csv") if CRIME_DATA_DIR else "",
...
try:
    ...
    print(f"⚠️ Parquet önizleme okunamadı: {e}")    m = {c.lower(): c for c in ci_names}
    for cand in candidates:
        if cand.lower() in m:
            return m[cand.lower()]
    return None
