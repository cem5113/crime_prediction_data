...
# En güncel CSV (09→00) -> İlk/Son kayıt aralığı bunun üzerinden okunacak
latest_csv = None
...
if latest_csv is not None:
    try:
        # ⚠️ Büyük dosyada çökmesin diye SADECE zaman kolonunu okumaya çalış
        ...
        # Header'ı görmek için 0 satır oku
        cols = pd.read_csv(latest_csv, nrows=0, low_memory=False).columns.tolist()
        time_col = next((c for c in time_candidates if c in cols), None)

        if time_col:
            s = pd.read_csv(latest_csv, usecols=[time_col], low_memory=False)[time_col]
