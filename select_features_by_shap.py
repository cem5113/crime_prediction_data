...
# sf_crime feature set'i için minimum beklenen kolonlar
...
def load_shap(shap_csv: str, top_k: int) -> list[str]:
    ...
    # GEOID ve event_hour yoksa en başa ekle (genelde gerekli)
...
def make_pipeline(X_cols: list[str], df: pd.DataFrame) -> tuple[Pipeline, list[str]]:
    ...
    # GEOID'i kategorik olarak tut
    if "GEOID" in num_cols:
        num_cols.remove("GEOID")
        if "GEOID" not in cat_cols:
...
def evaluate(df: pd.DataFrame, feats: list[str]) -> float:
    # Train/Test: son %20 test (zaman bazlı split)
...
def main():
    ...
    ap.add_argument(
        "--data",
        required=True,
        help="sf_crime feature dataset (örn: sf_crime_09.parquet veya CSV)",
    )
    ap.add_argument(
        "--shap_csv",
        required=True,
        help="SHAP importance CSV (feature, importance vb.)",
    )
    ...
    for k in range(args.kmin, args.kmax + 1, args.step):
        ...
        results.append(
            {"k": k, "ap": ap_k, "used": len([c for c in feats_k if c in df.columns])}
        )
        if ap_k > best_ap:
