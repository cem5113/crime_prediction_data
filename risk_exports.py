...
def export_risk_tables(df, y, proba, threshold, out_prefix=""):
    ...
    _stack = pd.concat([
        risk[["GEOID", "date", "top1_category", "top1_expected"]].rename(
            columns={"top1_category": "cat", "top1_expected": "lam"}),
        risk[["GEOID", "date", "top2_category", "top2_expected"]].rename(
            columns={"top2_category": "cat", "top2_expected": "lam"}),
        risk[["GEOID", "date", "top3_category", "top3_expected"]].rename(
            columns={"top3_category": "cat", "top3_expected": "lam"}),
    ], ignore_index=True)
