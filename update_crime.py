...
if os.path.exists(csv_path):
    ...
    print(f"📂 Existing data loaded: {len(df_old)} rows (latest date: {latest_date})")
else:
...
# === 6. VERİ TEMİZLİĞİ ve GEOID EŞLEMESİ ===
if new_data:
...
# === 7. BİRLEŞTİR, ETİKETLE, ÖZELLİKLERİ EKLE ===
...
print(f"\n✅ Güncellenmiş suç verisi kaydedildi: {len(df_all)} satır")

# === 9. TAHMİN GRIDİ ve LABEL ÜRETİMİ ===
...
# === Temiz ve eksiksiz grid üretimi
...
summary_path = "/content/drive/MyDrive/crime_data/sf_crime_grid_summary_labeled.csv"
full_path = "/content/drive/MyDrive/crime_data/sf_crime_grid_full_labeled.csv"

# === Kaydet
df_final_clean.to_csv(summary_path, index=False)

# Eksik kombinasyonları Y=0 ile tamamla
existing_combinations = df_final_clean[group_cols]
merged = full_grid.merge(existing_combinations.drop_duplicates(), on=group_cols, how="left", indicator=True)
missing = merged[merged["_merge"] == "left_only"].drop(columns=["_merge"])
missing["crime_count"] = 0
missing["Y_label"] = 0

df_full = pd.concat([df_final_clean, missing], ignore_index=True)
df_full.to_csv(full_path, index=False)

print(f"✅ sf_crime_grid_summary_labeled.csv: {df_final_clean.shape[0]} satır (Y=1)")
print(f"✅ sf_crime_grid_full_labeled.csv: {df_full.shape[0]} satır (Y=1 + Y=0)")
